...
export class CustomGraphics extends Component {
  ...
  @property({ tooltip: '漸層起始顏色' }) gradientStart: Color = new Color(255, 0, 0, 255);
  @property({ tooltip: '漸層結束顏色' }) gradientEnd: Color = new Color(0, 0, 255, 255);
  ...
  private drawShapes() {
    ...
    // 形狀 1: 矩形
    g.lineWidth = 2;
    g.rect(156, 326, 188, -250);
    g.fill();
    /* stroke overlay later */
    minX = Math.min(minX, 156); minY = Math.min(minY, 76); maxX = Math.max(maxX, 344); maxY = Math.max(maxY, 326);
    ...
    s.rect(156, 326, 188, -250); s.stroke();
  }
}
